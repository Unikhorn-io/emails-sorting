...
sent_file = "sent.csv"     # List of contacts who received an email
output_file = "output.csv"     # List of contacts who did not receive an email
...
with open(complete_file, "r", encoding="utf-8") as f, open(output_file, "w", encoding="utf-8", newline="") as out:
    ...
    for row in reader:
        ...
        if email not in emails_sent:
            writer.writerow(row)  # Write the row in the output file
